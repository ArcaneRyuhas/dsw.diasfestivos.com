...
const API_KEY = "2f503969-acef-4ec5-a0f5-a0d9b139a14c";
...
function consultar(){
    ...
    var URL_CONSULTA = API_URL_BASE+"holidays?"+"language=es"+"&key="+API_KEY+"&country="+cbx_pais.value+"&year="+txt_anio.value+"&month="+cbx_mes.value;
    ...
    request.onerror = function(){
        alert("El API KEY solo permite consultar los dias feriados del año inmediato anterior");
    }
...
function mostrarDiasFestivos(diasFestivos){
    ...
    titulo.innerText = "Dias festivos de " + cbx_pais.options[cbx_pais.selectedIndex].text + " en "+ cbx_mes.options[cbx_mes.selectedIndex].text + " del año " + txt_anio.value; 
...
function generarTarjeta(datos){
    console.log(datos);
    return '<div class="tarjeta"><h3>'+datos.name+'</h3><h4>'+datos.date+'</h4><br>' + datos.weekday.date.name+"</div>";
}